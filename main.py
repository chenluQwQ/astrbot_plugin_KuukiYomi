...
class KuukiYomi(Star):
    ...
    async def _judge_and_route(self, event: AstrMessageEvent, air_cfg: dict, is_pk_hit: bool):
        ...
        # ── 阈值过滤：分数太低直接沉默 ──
        threshold = float(self._cfg_group("air_reading", "score_threshold", 4.0))
        if overall < threshold and action != "silent":
            logger.debug(f"[KuukiYomi] 分数 {overall:.1f} < 阈值 {threshold}，覆盖为沉默")
            action = "silent"
